...
CSV_OK = """tipo,nombre,padre,activo_tipo,plataforma,version,sistema_operativo,ip,descripcion,estado,etiquetas,usuario_acceso,password,servicio,puerto
servidor,srv-imp,,,,,Proxmox,10.0.0.6,Host,activo,producción,,,,
hipervisor,pve-imp,,,Proxmox VE,8.2,,10.0.0.7,Nodo,activo,,,,,
...
def test_importacion_informa_errores_por_fila_sin_abortar(client):
    autenticar_admin(client)
    csv_mixto = (
        "tipo,nombre,padre,activo_tipo,usuario_acceso,password\n"
        "servidor,srv-ok,,,,\n"                       # válido (dedicado)
        "hipervisor,pve-ok,,,,\n"                     # válido (nivel superior)
        "vm,vm-huerfana,tampoco-existe,,,\n"          # padre inexistente → error
    )
    r = _importar(client, csv_mixto)
    assert r.status_code == 200
    assert "2 de 3" in r.text
    assert "no encontrado" in r.text  # se informa el error
    db = sesion_bd()
    try:
        from app.models import Hipervisor, MaquinaVirtual, ServidorFisico

        assert db.scalar(select(func.count(ServidorFisico.id))) == 1
        assert db.scalar(select(func.count(Hipervisor.id))) == 1
        assert db.scalar(select(func.count(MaquinaVirtual.id))) == 0
    finally:
        db.close()
