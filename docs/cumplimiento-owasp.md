...
|---|---|---|
| **A01 — Broken Access Control** | RBAC por rol en cada ruta (`requiere_permiso`) **más control de acceso por objeto** (`app/access.py`): admin/operador acceso total, auditor lectura sin revelar, y **analista default-deny** que solo ve/usa los activos concedidos; los rechazos quedan auditados y persisten pese al rollback; CSRF en todo POST; las contraseñas nunca aparecen en listados (solo vía endpoints auditados); cookies `SameSite=Strict`. | `app/deps.py`, `app/rbac.py`, `app/access.py` · `test_auditor_no_puede_revelar_ni_gestionar`, `test_concesion_ver_permite_ver_pero_no_revelar`, `test_activo_ajeno_responde_404`, `test_editar_sin_csrf_rechazado` |
...
| Riesgo | Medidas implementadas | Evidencia / pruebas |
|---|---|---|
| **API1 — Broken Object Level Authorization** | Autorización por objeto explícita: para el rol analista, cada acceso a un activo o credencial verifica una concesión vigente; un activo no concedido responde **404** (sin filtrar existencia) y un nivel insuficiente **403**. admin/operador conservan acceso de equipo. | `app/access.py` · `test_activo_ajeno_responde_404`, `test_concesion_sin_herencia`, `test_revocacion_es_inmediata` |
